...
router.post("/converter", (req, res) => {
  ...
  MainLink.findOne({ realUrl: linkImported }).then(link => {
    if (link) {
      ...
      // return res.status(400).json(errors)
      console.log(
        "this link is already exist in our database and converted before."
      );
      res.json({
        message:
          "this link is already exist in our database and converted before.",
...
router.post(
  ...
  (req, res) => {
    ...
    // reduce the link
    // res.json('it works')
    if (linkComming === undefined) {
      console.log("undefined link");

      return 0;
    }
    User.findOne({ _id: req.user._id }).then(user => {
      Link.findOne({ user: req.user.id, textlink: linkComming }).then(link => {
  ...
  (req, res) => {
    ...
    User.findOne({ _id: req.user._id }).then(user => {
      Link.findById(req.params.id)
        ...
        .catch(err =>
          res.status(404).json({ nopostfound: "No link found by that id" })
        );
    });
  }
);
      ...
      Link.findById(req.params.id)
        .then(link => {
          ...
          if (link.user.toString() !== req.user.id) {
            // link.user is an id and not string, so we used toString()
  ...
  (req, res) => {
    User.findOne({ _id: req.user._id }).then(user => {
      Link.findById(req.params.id)
        .then(link => {
          if (
            link.likes.filter(like => like.user.toString() === req.user.id)
              .length > 0
          ) {
            // check if likes array has the id of this user
            return res
              .status(400)
              .json((alreadyliked = "User already liked this link"));
          }
...
router.delete(
  ...
  (req, res) => {
    User.find().then(users => {
      // who wants to delete comment
      Link.findById(req.params.id)
        .then(link => {
          // which link contains this comment
          if (
            link.comments.filter(
          ...
          if (commentOwner !== null || linkCreator !== null) {
            const removeIndex = link.comments
